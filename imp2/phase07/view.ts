...
export const view = (model: Model) => 
    pipe(
        ...
        ({ playerEgg, eggnemies, bosses }) => [
            ...
            ...pipe(
                Array.map(bosses, (boss) => viewEgg(boss, model.bossSpriteSrc)),
                Array.flatten
            ),
            ...pipe(
                Array.map(eggnemies, (eggnemy) => viewEgg(eggnemy, model.eggnemySpriteSrc)),
                Array.flatten
            ),
            ...(model.gameState == "GameOver"? Array.empty(): viewEgg(playerEgg, model.playerSpriteSrc)), // spread empty array
            Canvas.Text.make({
                x: 50,
                y: 50,
...
const viewEgg = (egg: Egg , src: string) => 
    [
        Canvas.CanvasImage.make({
            // dimensions of images used are approximately twice that of
            // the rectangular dimensions of the egg.
                    x: egg.centerCoords.x - (egg.width),
                    y: egg.centerCoords.y - (egg.height),
                    src: src,
        }),
        Canvas.Text.make({
            x: egg.centerCoords.x - egg.width,
